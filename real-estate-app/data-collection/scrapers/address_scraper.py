...
class AddressScraper:
    """Парсер оголошень нерухомості з Address.ua"""

    def __init__(self):
        self.base_url = "https://www.address.ua"
        self.ua = UserAgent()
    ...
    def parse_listing(self, listing_html) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                'external_id': f"address_{int(time.time())}_{hash(listing_html) % 10000}",
                'source': 'address',
                'title': 'Test Address listing',
                'price_uah': 200000,
                'url': 'https://address.ua/test',
    ...
    def scrape_city_listings(self, city: str, max_pages: int = 1) -> List[Dict]:
        ...
        test_listings = [
            {
                'external_id': f"address_test_{i}",
                'source': 'address',
                'title': f'Тестове оголошення {i} в {city}',
                'price_uah': 200000 + i * 10000,
                'url': f'https://address.ua/test/{i}',
                'city': city,
                'scraped_at': datetime.utcnow().isoformat()
            } for i in range(2)
        ]
    ...
    def parse_listing(self, listing_html) -> Optional[Dict]:
        """Парсить одне оголошення з Address.ua"""
        try:
            # ID оголошення
            ...
            # Заголовок
            title_elem = listing_html.find('h2') or listing_html.find('h3') or listing_html.find('a', class_='title')
            ...
            logger.error(f"Помилка парсингу оголошення Address.ua: {e}")
            return None
    ...
    def _extract_price(self, price_text: str) -> int:
        ...
        try:
            if numbers:
                # Address.ua часто показує ціну за місяць
    ...
    def scrape_city_listings(self, city: str, max_pages: int = 3) -> List[Dict]:
        ...
        logger.info(f"Починаю збір оголошень Address.ua для міста: {city}")
        ...
        for page in range(1, max_pages + 1):
            ...
            try:
                ...
                # Знаходимо всі оголошення на сторінці - Address.ua використовує різні класи
                listings = []
                ...
                listings.extend(soup.find_all('article'))
        ...
        logger.info(f"Зібрано {len(all_listings)} оголошень Address.ua для {city}")
        return all_listings
